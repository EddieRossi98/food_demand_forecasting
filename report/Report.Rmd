...
For looking to main trends in the data we plot the **mean** number of orders **aggregated** by **center** and by **meal**.

```{r, echo = F, results = 'asis', out.width="60%", fig.align = 'center'}
title_a = paste("Mean orders of meal", 1062)
a = ggplot(aggregate_m(meal = 1062, dataset = X), aes(x=week, y=num_orders)) +
      xlab("Week") + ggtitle(title_a) + geom_line(color="darkred") +
      ylab("Number of orders") + theme_classic(base_size = 17)

title_b = paste("Mean orders in center", 77)
b = ggplot(aggregate_c(center = 77, dataset = X), aes(x=week, y=num_orders)) +
      geom_line(color="darkred") + xlab("Week") + ggtitle(title_b) + 
      ylab("Number of orders") + theme_classic(base_size = 17)
plot_grid(a, b, label_size = 12, ncol = 2)
```
# Time series Exploratory Analysis
